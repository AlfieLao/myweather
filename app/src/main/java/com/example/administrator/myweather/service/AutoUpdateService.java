...
//        int eightHour = 8*60*60*1000;
        int eightHour = 8*1000;
        long triggerAtTime = System.currentTimeMillis() +eightHour;
    ...
    private void updateWeather() {
        ...
        if (weatherString != null){
            ...
            HttpUtil.sendOkHttpRequest(weatherUrl, new Callback() {
                @Override
                public void onResponse(Call call, Response response) throws IOException {
                    String responseText = response.body().string();  //转化为Json字符串？
                    Weather weather = Utility.handleWeatherResponse(responseText);
                        if (weather != null && "ok".equals(weather.status)){
                            ...
                            Toast.makeText(AutoUpdateService.this,"获取天气信息失败",Toast.LENGTH_LONG).show();
                        }
                }
                @Override
                public void onFailure(Call call, IOException e) {
